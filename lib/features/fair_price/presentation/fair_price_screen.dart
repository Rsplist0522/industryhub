...
class FairPriceScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<FairPriceScreen> createState() =>
      _FairPriceScreenState();
}

class _FairPriceScreenState extends ConsumerState<FairPriceScreen> {
  final _product = TextEditingController(
    text: 'Aluminium machining offcuts',
  );
  ...
  static const _collectionTerms = <String>[
    'Buyer collects',
    'Seller delivers',
  ];
  ...
  void initState() {
    ...
    Future<void>.microtask(
      () => _loadMarketSignals(_product.text.trim()),
    );
  }
  ...
  void _applyInitialScenario() {
    ...
    final quantityValue =
        deal.quantityValue ?? _parseQuantity(deal.quantity);
    ...
    if (deal.askingPricePerKg != null &&
        deal.askingPricePerKg! > 0) {
  ...
  Future<void> _loadMarketSignals(String product) async {
    ...
    try {
      final commodity =
          await _marketPriceRepository.fetchLatestCommodity(product);
      final materialIndex =
          await _marketPriceRepository.fetchLatestMaterialIndex(product);
      final ppi =
          await _marketPriceRepository.fetchLatestMalaysiaPpi();
      final localPrices =
          await _marketPriceRepository.fetchLatestLocalListingPrices(
        product,
      );
  ...
  String? _requiredText(String? value) =>
      validateRequiredText(
        value,
        label: 'a material or product name',
      );
  ...
  _NegotiationResult _calculateResult({
    ...
    if (quantity >= 1000) {
      ...
      ceiling += 0.50;
      adjustments.add(
        'Smaller volume supports a handling allowance.',
      );
    } else {
      adjustments.add(
    ...
    if (_collection == 'Seller delivers') {
      ...
      ceiling -= 0.75;
      adjustments.add(
        'Seller delivery absorbs part of the logistics cost.',
      );
    } else {
      adjustments.add(
  ...
  _Benchmark _benchmarkFor(
    String product,
    double proposedPrice,
  ) {
    final prices = _localListingPrices
  ...
  double _roundToFiftySen(double value) =>
      (value * 2).round() / 2;
  ...
  Future<void> _sendChat() async {
    ...
    if (result == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
            'Prepare the negotiation before asking FairPrice.',
          ),
        ),
      );
      return;
...
''',
  ...
  Future<void> _saveRecommendation() async {
    ...
    try {
      await ref.read(appStateProvider.notifier).saveNegotiation(
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
            'Price recommendation saved to your profile.',
          ),
        ),
      );
    } catch (_) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
            'Price recommendation could not be saved. Please try again.',
          ),
        ),
      );
    }
  ...
  Future<void> _openDocumentation(String url) async {
    ...
    if (uri == null ||
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
            'The documentation link could not be opened.',
          ),
        ),
      );
    }
      ...
      actions: [
        IconButton(
          tooltip: _openedFromAcceptedDeal
              ...
              : 'Reset scenario',
          onPressed:
              (_isRunning || _isChatThinking) ? null : _resetScenario,
              ...
              children: [
                ...
                SizedBox(
                  ...
                  child: FilledButton.icon(
                    onPressed:
                        _isRunning ? null : _prepareNegotiation,
                    icon: _isRunning
                        ? const SizedBox(
                            width: 17,
                            height: 17,
            ...
            Row(
              ...
              children: [
                ...
                IconButton.filled(
                  tooltip: 'Send',
                  onPressed:
                      _isChatThinking ? null : _sendChat,
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              ...
              style:
                  AppTheme.dataStyle.copyWith(fontSize: 14),
            ),
          ...
          children: [
            Row(
              children: [
                ...
                const Expanded(
                  child: Eyebrow('EXTERNAL MARKET SIGNALS'),
                ),
                if (isLoading)
                  const SizedBox(
                    width: 16,
