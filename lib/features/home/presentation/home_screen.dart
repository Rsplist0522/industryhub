...
class _HomeScreenState extends ConsumerState<HomeScreen> {
  ...
  int get _unreadCount => _notifications.where(_isUnreadNotification).length;
  ...
  Future<void> _openNotification(DealRequestRecord request) async {
    ...
    final requesterAccepted = !isOwnerSide && request.status == 'ACCEPTED';

    final requesterRejected = !isOwnerSide && request.status == 'REJECTED';
          ...
          child: Column(
            ...
            children: [
              ...
              if (request.note.trim().isNotEmpty)
                _NotificationDetailLine(label: 'Message', value: request.note),
    ...
    return Material(
      ...
      child: ListTile(
        shape: RoundedRectangleBorder(
          borderRadius: BorderRadius.circular(8),
          side: BorderSide(color: unread ? AppColors.line : Colors.transparent),
        ),
        leading: Stack(
